...
import glob
import sys
import time as ttime  # Добавлено для задержки
...
def main():
    ...
    try:
        # ОЖИДАЕМ 5 СЕКУНД ПЕРЕД УСТАНОВКОЙ
        ttime.sleep(5)  # Даем время системе загрузиться
        
        # Установка правильных обоев сразу при запуске
        now = datetime.now()
        if should_use_day_wallpaper(now):
            wallpaper_path = get_random_wallpaper(DAY_WALLPAPERS_DIR)
            wallpaper_type = "day"
        ...
        set_wallpaper(wallpaper_path)
        print(f"Установлены {wallpaper_type} обои при запуске")
    ...
    # Держим скрипт активным (без консоли)
    try:
